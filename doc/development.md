...
By default, Webpack will build a combined [Purescript example](/examples/ByUrl.purs). This allows users to choose from different example contracts and the wallet to use. Make sure to follow the [instructions for setting up light wallets](./runtime#other-requirements) before running the examples. You can point Webpack to another Purescript entrypoint by changing the `ps-bundle` variable in the Makefile or in the `main` argument in the flake's `packages.ctl-examples-bundle-web`.
