...
, spagoPackages ? "${src}/spago-packages.nix"
  ...
  # shell.
  # `shell.packageLockOnly`: When set to true, the `node_modules` folder will
  # not be built when `npm` is used inside the shell.
, shell ? { }
, ...
}:
let
    ...
    , packageLockOnly ? false
      ...
      pkgs.mkShell {
        ...
        shellHook = ''
          ...
          export NPM_CONFIG_PACKAGE_LOCK_ONLY="${if packageLockOnly then "true" else "false"}"
        ''
        + shellHook;
      };
